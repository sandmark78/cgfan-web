---
title: "审查之下的女性角色生成"
slug: gemnana-2688
date: 2026-03-27
added: 2026-07-27T12:54:23+08:00
model: Midjourney
category: editorial
tags:
  - AI绘图
  - 提示词
difficulty: intermediate
source: "X / VL_Aschcroft"
sourceLink: "https://x.com/Project_VLA/status/2037534354176684347"
authorLink: "https://x.com/Project_VLA"
cover: /images/prompts/gemnana-2688.jpg
---

## Prompt

好吧，看来 @grok 的审查又开始抬头了，我猜，你的帖子目前还没有被标记。
但为了展示 @grok 的审核系统有多糟糕。这是用 #midjourney 制作的，然后又用 #NanoBananaPro 重新渲染。这两个生成器以对女性角色实行严格审查而闻名。
原始提示由 @art_muse 提供，见 ALT
#AIArt #PromptShare #AIArtistsCommunity

## 美学评分

- 平均分: 9.0
- 推荐: 是

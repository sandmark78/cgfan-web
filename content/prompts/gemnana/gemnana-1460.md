---
title: "Create a 2x2 grid 图像. In each frame i..."
slug: gemnana-1460
date: 2026-01-16
added: 2026-07-27T11:31:51+08:00
model: Common
category: style
tags:
  - AI绘图
  - 提示词
difficulty: intermediate
source: "https://gemnana.com/zh/case/1460.html"
sourceLink: "https://x.com/MrDavids1/status/2012224276900851827"
author: "X / Travis Davids"
authorLink: "https://x.com/MrDavids1"
cover: /images/prompts/gemnana-1460.jpg
---

## Prompt

创建一个 2x2 网格的图像。在每个画面中包含中景、特写、高角度向下拍摄，以及从鞋子处向上极端低角度的视角。并在每个画面中使用鱼眼镜头透视，以及带有透视缩短效果的美学姿势。角色穿着深红色飞行夹克、蓝色牛仔裤和白色耐克高帮运动鞋，在每个画面中摆出不同的搞怪表情。在每个画面周围加入白色手绘涂鸦元素。工作室摄影，背景为深蓝色。确保背景覆盖整个画面背景，且不使用圆形取景框。

## 美学评分

- 平均分: 8.2
- 推荐: 是

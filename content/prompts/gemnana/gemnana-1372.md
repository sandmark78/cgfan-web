---
title: "Create a collection of icons representin... 提示词"
slug: gemnana-1372
date: 2026-01-15
added: 2026-07-27T11:31:50+08:00
model: Common
category: style
tags:
  - AI绘图
  - 提示词
difficulty: intermediate
source: "https://gemnana.com/zh/case/1372.html"
sourceLink: "https://x.com/NanoBanana/status/2011857121835458929"
author: "X / Nano Banana Pro"
authorLink: "https://x.com/NanoBanana"
cover: /images/prompts/gemnana-1372.jpg
---

## Prompt

创建一组图标，代表 [主题]，它们属于同一主题。将它们放在 2x2 网格中（无分隔线）。背景为纯白色。将图标制作成 Risograph 印刷风格。无文字。无色彩失真。色彩鲜艳且不褪色。在色块填充内使用随机点画和沙状噪点纹理。每个图标都有粗黑色描边。

## 美学评分

- 平均分: 8.2
- 推荐: 是

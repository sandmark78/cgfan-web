...
export const dailyFeatures: DailyFeature[] = [
  {
    ...
    curatorNote: '这条提示词的精妙之处在于将技术需求拆解为可执行的模块化指令：从Gradio UI、Depth Anything V2模型推理，到CUDA/MPS/CPU自动切换、模型选择、分辨率控制、时序平滑，再到音频保留与ffmpeg导出，每个环节都给出了明确的边界条件。它没有依赖现有项目，而是直接要求Codex生成完整Python脚本，这对快速原型验证极有价值。特别值得称道的是对硬件加速的自动fallback设计，以及时序平滑降低闪烁的需求——这是深度视频转换中容易被忽略的细节。适合需要快速搭建本地工具的开发者。',
    highlight: '一条指令生成完整本地深度视频转换器，含硬件加速与时序平滑',
    technique: 'Gradio Web UI · 硬件加速自动适配 · 时序平滑降闪烁 · 功能模块化拆解',
    tip: '将复杂工具需求拆解为“UI+模型+硬件+后处理+导出”的独立子项，并明确平台条件分支，可大幅提升生成代码的完整性和可运行性。',
    tryChange: '将“Depth Anything V2”改为“MiDaS v3.1”，测试不同单目深度估计模型对生成结果的影响。',
  },
  {
    date: '2026-08-04',
